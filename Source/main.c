...
tList taskTable[zunxOS_PRO_COUNT];
...
** Descriptions         :   任务调度接口。zunxOS通过它来选择下一个具体的任务，然后切换至该任务运行。
** parameters           :   无
** Returned value       :   无
***********************************************************************************************************/
...
void tTimeTaskWait (tTask * task, uint32_t ticks)
{
    ...
    task->state |= zunxOS_TASK_STATE_DELAYED;
}
...
void tTaskSystemTickHandler (void) 
{
    ...
    if (--currentTask->slice == 0) 
    {
        ...
        {            
            ...
            // 重置计数器
            currentTask->slice = zunxOS_SLICE_MAX;
        }
    }
...
// 用于空闲任务的任务结构和堆栈空间
tTask tTaskIdle;
tTaskStack idleTaskEnv[zunxOS_IDLETASK_STACK_SIZE];
...
int main () 
{
    ...
    // 创建空闲任务
    tTaskInit(&tTaskIdle, idleTaskEntry, (void *)0, zunxOS_PRO_COUNT - 1, &idleTaskEnv[zunxOS_IDLETASK_STACK_SIZE]);
